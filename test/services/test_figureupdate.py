...
class TestUpdateFigureServiceStatus:
    ...
    @pytest.mark.parametrize("figure_id, player_id, game_started, turn, figure_exists, figure_player_id, new_status, expected_exception, expected_response", [
    ...
    def test_update_figure_service_status(self, mocker, figure_id, player_id, game_started, turn, figure_exists, figure_player_id, new_status, expected_exception, expected_response):
        ...
        if figure_exists:
            ...
            mock_figure = Mock(id=figure_id, type=FigureType.TYPE1, player=Mock(id=figure_player_id), game=Mock(started=game_started, turn=turn))
        ...
        mock_update_figure_status.return_value = Mock(id=figure_id, status=new_status)
        ...
        if isinstance(expected_exception, Exception):
            with pytest.raises(Exception, match=str(expected_exception)) as exc_info:
                instance.update_figure_service_status(figure_id, player_id, new_status)
        else:
            response = instance.update_figure_service_status(figure_id, player_id, new_status)
